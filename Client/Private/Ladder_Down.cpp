...
CLadder_Down::CLadder_Down(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
	: CGameObject(pDevice, pContext)
{
}

CLadder_Down::CLadder_Down(const CLadder_Down& rhs)
	: CGameObject(rhs)
{
}

HRESULT CLadder_Down::Initialize_Prototype()
{
	return S_OK;
}

HRESULT CLadder_Down::Initialize(void* pArg)
{
...
void CLadder_Down::LateTick(_float fTimeDelta)
{
	if (m_bInstancing)
		return;
...
void CLadder_Down::OnCollisionStay(CGameObject* pOther)
{
...
HRESULT CLadder_Down::Ready_Components(void* pArg)
{
	...
	m_LadderDesc.iStartHeight = 1;
	m_LadderDesc.iPlayerAnimIdx = Corvus_SD_Ladder_Climb_Start;
	XMStoreFloat4(&m_LadderDesc.vOffset, -m_pTransform->Get_GroundLook() * 0.98f + m_pTransform->Get_Up() * 0.1f);
	m_LadderDesc.pWorldMatrix = m_pTransform->Get_WorldMatrixPtr();
	return S_OK;
}


CLadder_Down* CLadder_Down::Create(ID3D11Device* pDevice, ID3D11DeviceContext* pContext)
{
	CLadder_Down* pInstance = new CLadder_Down(pDevice, pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX(TEXT("Failed To Created : CLadder_Down"));
...
CGameObject* CLadder_Down::Clone(void* pArg)
{
	CLadder_Down* pInstance = new CLadder_Down(*this);

	if (FAILED(pInstance->Initialize(pArg)))
	{
		MSG_BOX(TEXT("Failed To Cloned : CLadder_Down"));
		Safe_Release(pInstance);
	}
